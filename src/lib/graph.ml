...
let initialize_graph rows cols =
  ...
  let neighbors (x, y) =
    ...
    List.filter (fun (nx, ny) -> nx >= 0 && nx < cols && ny >= 0 && ny < rows) neighbor_positions
  in

  (* Create the graph by iterating over all possible positions in the grid *)
  let rec create_graph x y graph =
    if y >= rows then graph  (* Base case: finished all rows *)
    else if x >= cols then create_graph 0 (y + 1) graph  (* Go to next row *)
    else
      let pos = (x, y) in
      let neighbor_positions = neighbors pos in
      create_graph (x + 1) y (add_node graph pos neighbor_positions)  (* Add node and continue *)
  in
